...
# Computing the categorical values 
def get_categorical_column_names(dataframe: pd.DataFrame) -> None:
...
def impute_categorical_missing_values(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    ## Applying the  feature engineering for categorical values 
...
def preprocess_categorical_data(dataframe: pd.DataFrame) -> pd.DataFrame:
    dataframe = impute_categorical_missing_values(dataframe)
    encoder = get_one_hot_encoder(dataframe)
    encoded_df = encode_categorical_data(dataframe, encoder)
    return encoded_df

# Preprocessing the data
def preprocess(dataframe: pd.DataFrame) -> pd.DataFrame:
    processed_df = preprocess_continuous_data(dataframe)
    processed_df = preprocess_categorical_data(dataframe)
    return processed_df
